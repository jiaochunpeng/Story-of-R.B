...
def get_NPC_active_request(npc):
    result=db.execute_query(f"select quest_shortdesc from quests where lower(NPC_name)='{npc}' and status='active' limit 1")
    if len(result)>0:
        return result[0][0]
    else:
        return ''
...
def load_request(quest_shortdesc):
